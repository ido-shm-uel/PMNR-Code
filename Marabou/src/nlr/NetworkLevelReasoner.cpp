              ...
              MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PREIMAGE_APPROX )
    {
        ...
        lpFormulator.optimizeBoundsWithLpRelaxation(
            _layerIndexToLayer, true, layerIndicesToParameters );
              ...
              Options::get()->getMILPSolverBoundTighteningType() ==
                  MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_ALL ||
              Options::get()->getMILPSolverBoundTighteningType() ==
                  MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_RANDOM ||
              Options::get()->getMILPSolverBoundTighteningType() ==
                  MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_ONCE )
    {
        const Vector<double> optimalCoeffs = OptimalParameterisedSymbolicBoundTightening();
        Map<unsigned, Vector<double>> layerIndicesToParameters =
            getParametersForLayers( optimalCoeffs );
...
Vector<Vector<PolygonalTightening>> NetworkLevelReasoner::generatePolygonalTighteningsForPMNR()
{
    Vector<Vector<PolygonalTightening>> tighteningsVector =
        Vector<Vector<PolygonalTightening>>( {} );
    Vector<Vector<NeuronIndex>> neurons = selectPMNRNeurons();

    for ( unsigned k = 0; k < neurons.size(); ++k )
    {
        ...
        */
        ...
        tighteningsVector.append( tightenings );
    }
    return tighteningsVector;
}

Vector<Vector<NeuronIndex>> NetworkLevelReasoner::selectPMNRNeurons()
{
    switch ( Options::get()->getMILPSolverBoundTighteningType() )
    {
    case MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR:
    case MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_ONCE:
    {
        return selectPMNRNeuronsHeuristically();
        break;
    }
    case MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_ALL:
    {
        return selectPMNRNeuronsFromAllLayers();
        break;
    }
    ...
    default:
    {
        Vector<Vector<NeuronIndex>> emptyVector = Vector<Vector<NeuronIndex>>( {} );
        return emptyVector;
    }
    }
}

Vector<Vector<NeuronIndex>> NetworkLevelReasoner::selectPMNRNeuronsHeuristically()
{
    ...
    {
        const Vector<Vector<NeuronIndex>> emptyVector( {} );
        return emptyVector;
    }
    ...
    Vector<Vector<NeuronIndex>> neurons( { selectedNeurons } );
    return neurons;
}

Vector<Vector<NeuronIndex>> NetworkLevelReasoner::selectPMNRNeuronsFromAllLayers()
{
    // Select layer with maximal PMNR neuron score sum.
    Vector<Vector<NeuronIndex>> neurons( {} );
    const unsigned maxNeurons = GlobalConfiguration::PMNR_SELECTED_NEURONS;
    const Vector<unsigned> &candidateLayers = getLayersWithNonfixedNeurons();
    if ( candidateLayers.empty() )
    {
        const Vector<Vector<NeuronIndex>> emptyVector( {} );
    ...
    {
        Layer *layer = _layerIndexToLayer[layerIndex];
        Vector<NeuronIndex> nonfixedNeurons = layer->getNonfixedNeurons();
        unsigned nonfixedNeuronsCount = nonfixedNeurons.size();
        for ( unsigned i = 0; i < nonfixedNeuronsCount / maxNeurons; ++i )
        {
            Vector<NeuronIndex> selectedNeurons = Vector<NeuronIndex>( {} );
            for ( unsigned j = 0; j < maxNeurons; ++j )
            {
                unsigned nonfixedNeuronIndex = maxNeurons * i + j;
                if ( nonfixedNeuronIndex < nonfixedNeuronsCount )
                {
                    selectedNeurons.append( nonfixedNeurons[nonfixedNeuronIndex] );
                }
            }
            neurons.append( selectedNeurons );
        }
    }
    return neurons;
}

Vector<Vector<NeuronIndex>> NetworkLevelReasoner::selectPMNRNeuronsRandomly()
{
...
const Vector<PolygonalTightening> NetworkLevelReasoner::OptimizeParameterisedPolygonalTightening()
{
    ...
    const Vector<Vector<PolygonalTightening>> &selectedTightenings =
        generatePolygonalTighteningsForPMNR();
    Vector<PolygonalTightening> optimizedTightenings = Vector<PolygonalTightening>( {} );
    for ( unsigned i = 0; i < selectedTightenings.size(); ++i )
    {
        for ( unsigned j = 0; j < selectedTightenings[i].size(); ++j )
        {
            PolygonalTightening tightening = selectedTightenings[i][j];
            bool maximize = ( tightening._type == PolygonalTightening::LB );
            double feasibiltyBound =
                maximize ? FloatUtils::infinity() : FloatUtils::negativeInfinity();
            ...
            tightening._value = OptimizeSingleParameterisedPolygonalTighteningWithBranching(
                tightening, optimizedTightenings, maximize, bound, i );
    ...
    unsigned idx )
{
    ...
    Vector<Vector<NeuronIndex>> selectedNeurons = selectPMNRNeurons();
    Vector<NeuronIndex> neurons = Vector<NeuronIndex>( {} );
    for ( const auto &index : selectedNeurons[idx] )
    {
    ...
    const Map<NeuronIndex, unsigned> &neuronToBranchIndex )
{
    ...
    unsigned maxIterations = GlobalConfiguration::GENERALIZED_INVPROP_MAX_ITERATIONS;
    double gammaStepSize = GlobalConfiguration::GENERALIZED_INVPROP_STEP_SIZE;
    double epsilon = GlobalConfiguration::DEFAULT_EPSILON_FOR_COMPARISONS;
    double weightDecay = GlobalConfiguration::GENERALIZED_INVPROP_WEIGHT_DECAY;
    double lr = GlobalConfiguration::GENERALIZED_INVPROP_LEARNING_RATE;
    ...
    Vector<double> gamma( gammaDimension, GlobalConfiguration::GENERALIZED_INVPROP_INITIAL_GAMMA );
...
void NetworkLevelReasoner::initializePMNRScoreMap()
{
    ...
    for ( const auto &pair : _layerIndexToLayer )
    {
        ...
        {
            _neuronToPMNRScores.insert( index, calculatePMNRNSSEScore( index ) );
        }
    }
}

double NetworkLevelReasoner::calculatePMNRNSSEScore( NeuronIndex index )
{
...
{
    // When using NSSE heuristic, all implemented activations could be branched before INVPROP.
    if ( Options::get()->getMILPSolverBoundTighteningType() ==
             MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR ||
         Options::get()->getMILPSolverBoundTighteningType() ==
             MILPSolverBoundTighteningType::BACKWARD_ANALYSIS_PMNR_ALL ||
